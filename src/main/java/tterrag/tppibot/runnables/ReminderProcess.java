...
public class ReminderProcess implements Runnable {
    ...
    public ReminderProcess(PircBotX bot, String... defaults) {
        ...
        reminderMap = gson.fromJson(mapConfig.getText(), new TypeToken<Map<String, Boolean>>() {
        }.getType());
        ...
        reminders = gson.fromJson(remindersConfig.getText(), new TypeToken<List<String>>() {
        }.getType());
        ...
        delayMap = gson.fromJson(delayConfig.getText(), new TypeToken<Map<String, RemindTime>>() {
        }.getType());
            ...
            try {
                ...
                if (bot.isConnected() && !bot.getUserChannelDao().getAllChannels().isEmpty()) {
                    ...
                    for (Channel channel : bot.getUserBot().getChannels()) {
                        ...
                        String reminder = reminders.get(remind.getIndex() % reminders.size()); // make sure removal hasn't shifted indeces

                        if (time - remind.getInit() > remind.getTime() && isRemindEnabledFor(channel.getName())) {
                            remind(channel, reminder);
                            delayMap.put(channel.getName(), new RemindTime(time, Main.reminderCommand.delayMap.get(channel.getName()), nextIndex(remind.getIndex())));
                        }
                    }
                } else {
    ...
    public void setDelay(String name, long millis) {
        if (delayMap.get(name) == null) {
            ...
            delayMap.get(name).setTime(millis);
        }
    }
